...
# 2. Nginx 웹 서버 컨테이너 백그라운드 실행 (메인 실습)
docker run -d --name web -p 8081:80 -v /opt/hello:/usr/share/nginx/html nginx:latest

# 3. 웹 서버 정상 동작 검증 (터미널에 'Hello from the host!'가 출력되면 성공)
curl http://localhost:8081/test.txt


# 4. [참고] 환경변수를 주입하여 컨테이너 실행하기
# 이미지 태깅 (my-app 으로 이미지 태깅)
